...
function feedImagens(){
    $.ajax({
        async: false,
        type: 'GET',
        url: "/Trabalho_F/php/getFeed.php?",
...
function listFotos(){
    ...
    feedImagens();
    if(counts==3 && grid.childElementCount>=1){
        var h3 = document.createElement("h3");
        h3.innerHTML="No more content avaible on your feed"; 
        h3.classList="lead";
        grid.appendChild(h3);
        counts=4;
    }
    else if(data.imagens=="erro"){
        console.log("Erro ");
    }
    else if(data.imagens=="noImages" && grid.childElementCount==0 && counts==3){
        var h3 = document.createElement("h5");
        h3.innerHTML="Your Feed Ins't Working, Thankfully You Can Make it Work by Following more People"; //muda isto para ser mais completo
        h3.classList="lead";
        grid.appendChild(h3);
        counts=4;
    }
        ...
        if(aceitavel==true){
            for(var i=0;i<data.imagens.length;i++ ){
                ...
                rowUsername.classList="py-2 col-8 col-md-8 pl-1";
                rowUsername.style="padding-bottom: .10rem!important;";
                ...
                path_img = data.imagens[i].url_image;
                imgs = path_img.replace('c:/wamp64/www','');
    ...
    if(grid.childElementCount<=2 && counts<=3){
        listFotos();
    }
}
...
window.onscroll = function(ev) {
 ...
 $(window).on("unload", function(e) {
    $.get("/Trabalho_F/php/deleteFeedS.php?");
});